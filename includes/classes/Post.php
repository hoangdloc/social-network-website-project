...
class Post
{
	...
	public function submitPost($body, $user_to, $imageName)
	{
		...
		if ($check_empty != "") {
			...
			//insert post 
			$query = mysqli_query($this->con, "INSERT INTO tblPost VALUES(NULL, '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0', '$imageName')");
			...
			$update_query = mysqli_query($this->con, "UPDATE tblUser SET num_posts='$num_posts' WHERE username='$added_by'");
				...
				new new newer newest next no nobody non none not nothing now nowhere number
	...
	{
		if ($term != '') {
			$query = mysqli_query($this->con, "SELECT * FROM tblTrend WHERE title = '$term'");

			if (mysqli_num_rows($query) == 0)
				$insert_query = mysqli_query($this->con, "INSERT INTO tblTrend(title, hits) VALUES ('$term', '1')");
			else
			$insert_query = mysqli_query($this->con, "UPDATE tblTrend SET hits = hits + 1 WHERE title = '$term'");
		}
	}
